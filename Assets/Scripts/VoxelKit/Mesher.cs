...
namespace VoxelKit
{
	static class Mesher
	{
		...
		private static bool IsFaceSetEmpty(VoxelKit.Shape.FaceSet faceSet)
		{
			...
			for (int x = 0; x < sizeX; x++)
			{
				for (int y = 0; y < sizeY; y++)
				{
					for (int z = 0; z < sizeZ; z++)
					{
						...
						if (!TryGetDefinition(cells[x, y, z], registry, predicate, out VoxelKit.Definition def, out VoxelKit.Cell cell))
						{
							continue;
						}

						VoxelKit.Shape shape = def.Shape;
						if (shape == null)
						{
							continue;
						}

						VoxelKit.Shape.FaceSet fs = shape.CollisionFaces;
		...
		private static bool IsCubicCollisionVoxel(VoxelKit.Cell cell, VoxelKit.Shape shape)
		{
			...
			for (int i = 0; i < VoxelKit.Shape.AxisPlanes.Length; i++)
			{
				...
				if (!fs.OuterShell.TryGetValue(localPlane, out VoxelKit.Face localFace)
					|| localFace == null
					|| !localFace.HasRenderablePolygons)
				{
					return false;
				...
				VoxelKit.Face worldFace = TransformFaceCached(localFace, cell.Orientation, cell.FlipOrientation);
				if (!VoxelKit.Utils.Geometry.IsFullFace(worldFace, worldPlane))
				{
					return false;
				}
			}
			...
			List<int> triangles)
		{
			...
			for (int p = 0; p < facePolygons.Count; p++)
			{
				...
				{
					VoxelKit.Face.Vertex v = faceVertices[i];
