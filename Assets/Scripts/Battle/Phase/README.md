...
## Purpose
Phase contains the battle phase framework plus the concrete battle flow phases. Each phase now lives in its own folder so phase-specific
supporting classes can stay colocated with the phase root.
...
Each concrete phase root uses the pattern `Erelia.Battle.Phase.<Name>.MainRoot`.
...
3. Add its id to `Id` and register it in `Registry`.
4. Request transitions from existing phases when appropriate.
