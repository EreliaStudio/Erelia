...
public static class BattleCellGraphBuilder
{
	...
	{
		private readonly CardinalHeightSet _source;
		private readonly VoxelOrientation _orientation;
		...
		{
			return ReferenceEquals(_source, p_other._source) && _orientation == p_other._orientation;
		}
		...
		{
			int hash = _source != null ? _source.GetHashCode() : 0;
			unchecked
			{
				hash = (hash * 397) ^ (int)_orientation;
			}
	...
	private readonly struct TraversalCandidate
	{
		...
		public TraversalCandidate(BattleCellGraph.Node p_node, float p_verticalGap, int p_verticalCellGap)
		{
			Node = p_node;
	...
	public static BattleCellGraph Build(BoardTerrainLayer p_board, VoxelRegistry p_voxelRegistry)
	{
		if (p_board == null)
		{
		...
		BattleCellGraph graph = new BattleCellGraph(p_board.SizeX, p_board.SizeY, p_board.SizeZ);
		...
		IReadOnlyList<BattleCellGraph.Node> allNodes = graph.AllNodes;
		for (int index = 0; index < allNodes.Count; index++)
		{
			BattleCellGraph.Node node = allNodes[index];

			AssignNeighbour(node, Vector2Int.right, BattleCellGraph.Node.Direction.PositiveX, CardinalHeightSet.Direction.PositiveX, CardinalHeightSet.Direction.NegativeX, graph, nodesByColumn, p_board, p_voxelRegistry);
			AssignNeighbour(node, Vector2Int.left, BattleCellGraph.Node.Direction.NegativeX, CardinalHeightSet.Direction.NegativeX, CardinalHeightSet.Direction.PositiveX, graph, nodesByColumn, p_board, p_voxelRegistry);
			AssignNeighbour(node, Vector2Int.up, BattleCellGraph.Node.Direction.PositiveZ, CardinalHeightSet.Direction.PositiveZ, CardinalHeightSet.Direction.NegativeZ, graph, nodesByColumn, p_board, p_voxelRegistry);
			AssignNeighbour(node, Vector2Int.down, BattleCellGraph.Node.Direction.NegativeZ, CardinalHeightSet.Direction.NegativeZ, CardinalHeightSet.Direction.PositiveZ, graph, nodesByColumn, p_board, p_voxelRegistry);
		}
	...
	private static void AssignNeighbour(
		...
		BattleCellGraph.Node.Direction p_graphDirection,
		...
		VoxelRegistry p_voxelRegistry)
	{
		...
		for (int index = 0; index < candidates.Count; index++)
		{
			...
			if (!TryGetVerticalGap(p_node.Position, candidatePosition, p_exitDirection, p_entryDirection, p_board, p_voxelRegistry, out float verticalGap))
			{
				continue;
			}

			BattleCellGraph.Node candidateNode = p_graph.GetNode(candidatePosition);
		...
		out float p_height)
	{
		...
		if (!p_voxelRegistry.TryGetVoxel(cell.Id, out VoxelDefinition voxelDefinition) || voxelDefinition?.Shape == null)
		{
			return false;
		}
	...
	private static bool IsReachableCell(BoardTerrainLayer p_board, Vector3Int p_position, VoxelRegistry p_voxelRegistry)
	{
