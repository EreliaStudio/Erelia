...
public sealed class BoardRuntimeRegistry
{
	...
	public void Revalidate(BoardNavigationLayer p_navigationLayer)
	{
		_navigationLayer = p_navigationLayer;
		...
		BoardRuntimeRegistry previousRegistry = new BoardRuntimeRegistry();
		previousRegistry._navigationLayer = _navigationLayer;

		foreach (KeyValuePair<BattleObject, Vector3Int> entry in _positionsByObject)
		{
			previousRegistry._positionsByObject[entry.Key] = entry.Value;
		}

		foreach (KeyValuePair<Vector3Int, BattleUnit> entry in _unitsByPosition)
		{
			previousRegistry._unitsByPosition[entry.Key] = entry.Value;
		}

		foreach (KeyValuePair<Vector3Int, List<BattleInteractiveObject>> entry in _interactiveObjectsByPosition)
		{
			previousRegistry._interactiveObjectsByPosition[entry.Key] = new List<BattleInteractiveObject>(entry.Value);
		}

		Clear();
		RestoreObjects(previousRegistry);
	}

	public void RestoreObjects(BoardRuntimeRegistry p_previousRegistry)
	{
		if (p_previousRegistry == null)
		{
			return;
		}

		foreach (KeyValuePair<BattleObject, Vector3Int> entry in p_previousRegistry._positionsByObject)
		{
			BattleObject battleObject = entry.Key;
			Vector3Int position = entry.Value;
			...
			if (battleObject is BattleUnit battleUnit)
			{
				TryPlaceUnit(battleUnit, position);
			}
			else if (battleObject is BattleInteractiveObject interactiveObject)
			{
				TryAddInteractiveObject(interactiveObject, position);
			}
		}
	}
}