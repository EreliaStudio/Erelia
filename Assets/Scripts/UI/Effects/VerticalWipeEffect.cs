...
{
	public sealed class VerticalWipeEffect : Erelia.UI.ScreenTransitionEffect
	{
		...
		private IEnumerator Animate(float from, float to)
		{
			...
			while (time < duration)
			{
				...
				float w = Mathf.Lerp(from, to, t) * full.x;
				maskRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, w);
				maskRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, full.y);
				float offset = leftToRight ? -0.5f : 0.5f;
				maskRect.pivot = new Vector2(leftToRight ? 0f : 1f, 0.5f);
				maskRect.anchorMin = new Vector2(leftToRight ? 0f : 1f, 0.5f);
				maskRect.anchorMax = new Vector2(leftToRight ? 0f : 1f, 0.5f);
