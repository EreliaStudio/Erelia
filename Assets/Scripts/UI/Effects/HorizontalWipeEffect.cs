...
{
	public sealed class HorizontalWipeEffect : Erelia.UI.ScreenTransitionEffect
	{
		...
		private IEnumerator Animate(float from, float to)
		{
			...
			while (time < duration)
			{
				...
				float h = Mathf.Lerp(from, to, t) * full.y;
				maskRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, full.x);
				maskRect.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, h);
				maskRect.pivot = new Vector2(0.5f, bottomToTop ? 0f : 1f);
				maskRect.anchorMin = new Vector2(0.5f, bottomToTop ? 0f : 1f);
				maskRect.anchorMax = new Vector2(0.5f, bottomToTop ? 0f : 1f);
				maskRect.anchoredPosition = new Vector2(0f, 0f);
				yield return null;
			}
		}
