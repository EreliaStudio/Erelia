...
namespace Erelia.Exploration.World
{
	...
	{
		private readonly Dictionary<Erelia.Exploration.World.Chunk.Coordinates, Erelia.Exploration.World.Chunk.ChunkData> chunks = new Dictionary<Erelia.Exploration.World.Chunk.Coordinates, Erelia.Exploration.World.Chunk.ChunkData>();
		...
		public IReadOnlyDictionary<Erelia.Exploration.World.Chunk.Coordinates, Erelia.Exploration.World.Chunk.ChunkData> Chunks => chunks;
		...
		public Erelia.Exploration.World.Chunk.ChunkData GetOrCreateChunk(Erelia.Exploration.World.Chunk.Coordinates coordinates)
		{
			if (chunks.TryGetValue(coordinates, out Erelia.Exploration.World.Chunk.ChunkData existing))
			{
				return existing;
			}

			var chunk = new Erelia.Exploration.World.Chunk.ChunkData();
		...
		public void SaveChunk(Erelia.Exploration.World.Chunk.Coordinates coordinates)
		{
			...
			if (!chunks.TryGetValue(coordinates, out Erelia.Exploration.World.Chunk.ChunkData chunk) || chunk == null)
			{
				return;
			}
			...
			{
				throw new System.InvalidOperationException("[Erelia.Exploration.World.WorldState] Chunk directory is not set.");
			}
		...
		public void Save(string path)
		{
			...
			var saveFile = new WorldSaveFile
			{
			...
			Erelia.Core.Utils.JsonIO.Save(path, saveFile, true);
		}
		...
		public bool Load(string path)
		{
			...
			WorldSaveFile saveFile = Erelia.Core.Utils.JsonIO.Load<WorldSaveFile>(path);
			if (saveFile == null)
			{
				Debug.LogWarning($"[Erelia.Exploration.World.WorldState] Save file not found or invalid at '{path}'.");
				return false;
			}

			string baseDirectory = Path.GetDirectoryName(path);
			string resolvedChunkDir = ResolvePath(baseDirectory, saveFile.ChunkDirectory);
			SetChunkDirectory(resolvedChunkDir);

			chunkGenerator = null;
			generatorType = saveFile.GeneratorType;
			generatorDataPath = saveFile.GeneratorDataPath;
			...
			if (!string.IsNullOrEmpty(generatorType))
			{
				...
				{
					Debug.LogWarning($"[Erelia.Exploration.World.WorldState] Generator type '{generatorType}' could not be resolved.");
				}
				else
				{
		...
		public void SaveAllChunks()
		{
			...
			foreach (KeyValuePair<Erelia.Exploration.World.Chunk.Coordinates, Erelia.Exploration.World.Chunk.ChunkData> entry in chunks)
			{
		...
		[System.Serializable]
		private sealed class WorldSaveFile
		{
