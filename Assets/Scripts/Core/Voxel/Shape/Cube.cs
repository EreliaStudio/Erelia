...
{
	[Serializable]
	public class Cube : Erelia.Core.Voxel.Shape
	{
		[Header("Textures")]
		...
		private Dictionary<AxisPlane, Erelia.Core.Voxel.Face> ConstructOuterShell()
		{
			...
			Erelia.Core.Voxel.Utils.SpriteUv.GetSpriteUvRect(spritePosY, out uvAnchor, out uvSize);
			uvA = uvAnchor;
			uvB = uvAnchor + new Vector2(uvSize.x, 0f);
			uvC = uvAnchor + uvSize;
			uvD = uvAnchor + new Vector2(0f, uvSize.y);
