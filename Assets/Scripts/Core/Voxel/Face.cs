	...
	public class Face
	{
		...
		public bool IsOccludedBy(Face other)
		{
			...
			for (int p = 0; p < Polygons.Count; p++)
			{
				...
				if (polygon != null && polygon.Count >= 3)
				{
					normal = Erelia.Core.Voxel.Utils.Geometry.GetNormal(polygon);

					if (normal.sqrMagnitude >= Erelia.Core.Voxel.Utils.Geometry.NormalEpsilon)
					{
						break;
					}
				}
			...
			for (int p = 0; p < Polygons.Count; p++)
			{
				...
				if (!Erelia.Core.Voxel.Utils.Geometry.IsPolygonContainedInUnion(polygon, other.Polygons, normal))
				{
					return false;
				}
			}
