...
## Purpose
VoxelKit is a small voxel pipeline used by Erelia to define voxel types, store voxel grids, and generate render/collision meshes.

Core flow:
1. A `Cell` stores voxel data for a grid location (id, orientation, flip).
2. The `Registry` maps a cell id to a `Definition` (ScriptableObject).
3. Each `Definition` owns a `Shape` that provides geometry faces.
4. The `Mesher` builds meshes from a `Cell[,,]` grid and a `Registry` to produce a `UnityEngine.Mesh`.
...
- `Registry` is a ScriptableObject that maps integer ids to `Definition` assets.
- `Definition` groups `Data` (gameplay flags) and a `Shape` (geometry).
- The registry is rebuilt on `OnEnable` and `OnValidate`.
- At runtime, `Registry.TryGet(id, out Definition)` resolves a cell id.
...
- `Data.Traversal` describes movement rules (Obstacle, Walkable).
...
5. Create a new `Definition` asset and select the shape.
6. Register the definition in the `Registry`.
...
- A voxel does not appear: check `Registry` id mapping and `Id < 0` rules.
