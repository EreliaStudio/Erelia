...
public sealed class ConsumeStatusTests : EffectTestBase
{
	...
	[Test]
	public void Apply_WithZeroConsumedStacks_DoesNotRemoveStatus()
	{
		BattleUnit target = CreateUnit();
		Status status = CreateStatus("focus");
		...
		Assert.That(target.Statuses[0].Value.Stack, Is.EqualTo(3));
	}

	[Test]
	public void Apply_WithNegativeConsumedStacks_DoesNotRemoveStatus()
	{
		BattleUnit target = CreateUnit();
		Status status = CreateStatus("focus");
