...
namespace Tests.Requirements.Meta
{
	public sealed class MetaRequirementTests
	{
		...
		// ── WithSpecificAbilityRequirement ────────────────────────────────────────

		[Test]
		public void WithAbility_EventsFromCorrectAbility_PassToChild()
		{
			Ability ability = ScriptableObject.CreateInstance<Ability>();
			var child = new DealDamageRequirement { RequiredAmount = 50 };
			var req = new WithSpecificAbilityRequirement { ChildRequirement = child, Ability = ability };
			var progress = new FeatRequirementProgress { Requirement = req };
		...
		[Test]
		public void WithAbility_EventsFromWrongAbility_Filtered()
		{
			Ability abilityA = ScriptableObject.CreateInstance<Ability>();
			Ability abilityB = ScriptableObject.CreateInstance<Ability>();
			var child = new DealDamageRequirement { RequiredAmount = 50 };
			var req = new WithSpecificAbilityRequirement { ChildRequirement = child, Ability = abilityA };
			var progress = new FeatRequirementProgress { Requirement = req };
		...
		[Test]
		public void WithAbility_MixedEvents_OnlyMatchingPassToChild()
		{
			Ability ability = ScriptableObject.CreateInstance<Ability>();
			Ability other = ScriptableObject.CreateInstance<Ability>();
			var child = new DealDamageRequirement { RequiredAmount = 50 };
			var req = new WithSpecificAbilityRequirement { ChildRequirement = child, Ability = ability };
			var progress = new FeatRequirementProgress { Requirement = req };
			...
			progress.RegisterEvents(new List<FeatRequirement.EventBase>
			{
				new DealDamageRequirement.Event { Amount = 30, SourceAbility = other },   // filtered out
				new DealDamageRequirement.Event { Amount = 50, SourceAbility = ability }  // passes through
			});
		...
		[Test]
		public void WithAbility_NullChild_ReturnsCurrentAdvancement()
		{
			Ability ability = ScriptableObject.CreateInstance<Ability>();
			var req = new WithSpecificAbilityRequirement { ChildRequirement = null, Ability = ability };
			var progress = new FeatRequirementProgress { Requirement = req };
