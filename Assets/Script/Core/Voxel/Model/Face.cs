	...
	public class Face
	{
		...
		public bool IsOccludedBy(Face other)
		{
			...
			for (int p = 0; p < Polygons.Count; p++)
			{
				...
				if (polygon != null && polygon.Count >= 3)
				{
					normal = Core.Utils.Geometry.GetNormal(polygon);
					if (normal.sqrMagnitude >= Core.Utils.Geometry.NormalEpsilon)
					{
						break;
					}
				}
			...
			for (int p = 0; p < Polygons.Count; p++)
			{
				...
				if (!Core.Utils.Geometry.IsPolygonContainedInUnion(polygon, other.Polygons, normal))
				{
					return false;
				}
			}
