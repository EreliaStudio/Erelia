...
namespace Utils.Mesher
{
	public class VoxelMaskRenderMesher : Utils.Mesher.Mesher
	{
		...
		public Mesh BuildMesh(Voxel.Model.Cell[,,] cells, Mask.Model.Cell[,,] maskCells)
		{
			if (cells == null)
			{
				return new Mesh();
		...
		private void AddVoxel(Voxel.Model.Cell[,,] cells, Mask.Model.Cell[,,] maskCells, int x, int y, int z)
		{
			...
			if (!TryGetMaskCell(maskCells, x, y, z, out Mask.Model.Cell maskCell) || maskCell == null)
			{
				return;
			}
			...
			IReadOnlyDictionary<Voxel.Model.FlipOrientation, List<Voxel.Model.Face>> maskFaces = shape.MaskFaces;
			if (maskFaces == null || maskFaces.Count == 0)
			{
				return;
			}

			if (!maskFaces.TryGetValue(flipOrientation, out List<Voxel.Model.Face> maskFacesForFlip))
			{
				maskFaces.TryGetValue(Voxel.Model.FlipOrientation.PositiveY, out maskFacesForFlip);
			}
			...
			{
				Voxel.Model.Face rotatedFace = TransformFaceCached(maskFacesForFlip[i], orientation, Voxel.Model.FlipOrientation.PositiveY);
				AddFaceWithSprite(rotatedFace, position, maskSprite, vertices, triangles, uvs);
			}
		}

		private bool TryGetMaskSprite(Mask.Model.Cell maskCell, out Sprite sprite)
		{
			sprite = null;
			...
			Mask.Model.Value maskValue = maskCell.Masks[maskCell.Masks.Count - 1];
		...
		private bool TryGetMaskCell(Mask.Model.Cell[,,] cellPack, int x, int y, int z, out Mask.Model.Cell cell)
		{
			cell = default;
			if (cellPack == null ||
		...
		public static Mesh Build(Voxel.Model.Cell[,,] cells, Mask.Model.Cell[,,] maskCells)
		{
