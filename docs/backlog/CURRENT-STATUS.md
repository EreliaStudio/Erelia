...
ST-001-09 is **Ready** and implementation is in progress. Its Collection/Provider asynchronous ownership and all public Server/protocol behavior are resolved. Sparkle Version-0.1.3 provides the generic manually-settled `spk::Task<TResult>`, direct-callable WorkerPool execution, completion subscriptions, thread-safe ContractProvider, and `spk::TaskGroup<TResult>` needed by the selected design.
