...
The internal batch size is fixed at 1024 coordinates as a TerrainNode implementation constant rather than configuration or protocol state. Since the current Chunk Request maximum is also 1024, every valid request currently maps to one Collection batch. The generic diagnostic payload is fixed to severity + human-readable message only, with later contextual extensions explicitly left for future work. Its public type is fixed as `Networking::Diagnostic` in the generic networking layer. Remaining diagnostic blockers are the severity numeric contract, byte encoding, correlation rule, and message-type value; lifecycle/reply/disconnect/shutdown behavior also remains unresolved. Exception-to-message mapping is fixed: `spk::Exception::message()`, otherwise `std::exception::what()`, otherwise exactly `"Unknown acquisition failure"`.
