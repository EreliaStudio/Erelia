...
ST-001-09 implementation preparation exposed one remaining public failure-semantic blocker: a true Collection batch/outer TaskGroup aggregation failure has no valid `BatchResult`, so its Client-visible reply behavior must be chosen before the terrain handler can be completed. Sparkle Version-0.1.3 provides the generic manually-settled `spk::Task<TResult>`, direct-callable WorkerPool execution, completion subscriptions, thread-safe ContractProvider, and `spk::TaskGroup<TResult>` needed by the selected design.
