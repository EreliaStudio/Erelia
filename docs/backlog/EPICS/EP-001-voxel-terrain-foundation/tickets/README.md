...
**ST-001-04 — First terrain Definition and Shape contract** is **In Progress** on `feat/st-001-04-definition-shape-contract` / PR #11. The production implementation and acceptance/regression coverage are technically complete. CI run #127 passed clang-format, Linux Core/Server Debug + Release, Windows Core/Server Debug + Release, and Windows Client Debug + Release. The Linux `std::atomic<spk::UUID>` link requirement is satisfied through transitive `libatomic` linkage without changing the approved lazy-cache publication contract. The ticket remains In Progress only until the Definition-of-Done project-owner approval is explicitly recorded; PR #11 remains unmerged.
