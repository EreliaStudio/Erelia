...
**ST-001-05 — Voxel::Volume Message serialization** is **Ready** on `feat/st-001-05-voxel-volume-message-serialization`. OQ-037 and DR-017 now fix the Sparkle-native wire contract, exact field order, contiguous Cell block, derived Cell count, validation rules, decode failure semantics, ownership, and deterministic-byte scope.
