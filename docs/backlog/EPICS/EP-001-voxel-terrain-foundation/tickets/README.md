...
**ST-001-04 — First terrain Definition and Shape contract** is **In Progress** on `feat/st-001-04-definition-shape-contract` / PR #11. CI run #127 passed the complete matrix after the original formatting/Linux failures were fixed. The project owner subsequently requested that the cache not introduce a `libatomic` dependency, so the final implementation publishes completed cache entries with a guaranteed lock-free `std::atomic_flag` and keeps `spk::UUID` as immutable ordinary data. The final no-`libatomic` variant is being revalidated. Project-owner Definition-of-Done approval remains pending; PR #11 remains unmerged.
