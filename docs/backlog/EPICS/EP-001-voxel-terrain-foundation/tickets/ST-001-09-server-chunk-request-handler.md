...
Finalized Response objects remain Message-backed. Builder-side temporary Success/Failure containers are discarded after encoding. The refined Response payload contains one `std::uint32_t failureOffset` header, then sorted Success entries, then sorted Failure entries. Success occupies `[4, failureOffset)` and Failure occupies `[failureOffset, message.size())`; `failureOffset == 4` means no Success entries and `failureOffset == message.size()` means no Failure entries.
