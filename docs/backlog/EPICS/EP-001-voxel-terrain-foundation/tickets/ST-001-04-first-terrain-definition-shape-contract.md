...
- a retained immutable shared reference to its resolved `Voxel::Shape`;
...
The Definition must retain the resolved Shape lifetime; use shared immutable ownership consistent with this contract.
...
On Definition-catalog construction, automatically create ID 0 with:
...
- Definition ID 0 always means catalog-provided Air.
...
- Definitions retain immutable shared Shape ownership so Shape lookup is not repeated and the referenced Shape remains alive.
- oriented polygon arrays live inside their Shape.
- references returned by `orientedPolygons()` and subcatalog `at`/operator[] are non-owning and may not outlive their owning Shape/catalog.
- `tryGet` pointers follow the same owner-lifetime rule.
...
- Air has null/no Shape and no slots;
...
- Definition keeps its resolved Shape alive through its approved shared immutable reference.
- non-owning catalog/oriented-array references are used only while their owner is alive.
