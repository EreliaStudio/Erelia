...
namespace
{
	...
	{
		const std::int32_t quotient = value / erelia::core::terrain::chunkExtent;
		const std::int32_t remainder = value % erelia::core::terrain::chunkExtent;
	...
	[[nodiscard]] constexpr std::int32_t floorModuloByChunkExtent(std::int32_t value) noexcept
	{
		...
		return remainder < 0 ? remainder + erelia::core::terrain::chunkExtent : remainder;
	}
}

spk::Vector3Int erelia::core::terrain::toChunkCoordinate(const spk::Vector3Int &globalCell) noexcept
{
	return {
...
spk::Vector3Int erelia::core::terrain::toLocalCoordinate(const spk::Vector3Int &globalCell) noexcept
{
	return {
