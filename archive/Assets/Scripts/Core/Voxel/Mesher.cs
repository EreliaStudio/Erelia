...
namespace Erelia.Core.Voxel
{
	static class Mesher
	{
		public static readonly Func<Erelia.Core.Voxel.VoxelDefinition, bool> AnyVoxelPredicate = (definition) => true;

		public static readonly Func<Erelia.Core.Voxel.VoxelDefinition, bool> OnlyObstacleVoxelPredicate =
			(definition) => definition.Properties.Traversal == Erelia.Core.Voxel.Traversal.Obstacle;

		public static readonly Func<Erelia.Core.Voxel.VoxelDefinition, bool> OnlyWalkableVoxelPredicate =
			(definition) => definition.Properties.Traversal == Erelia.Core.Voxel.Traversal.Walkable;

		static public UnityEngine.Mesh BuildRenderMesh(
			Erelia.Core.Voxel.Cell[,,] cells,
			Erelia.Core.Voxel.VoxelRegistry registry,
			Func<Erelia.Core.Voxel.VoxelDefinition, bool> predicate)
		{
			...
			for (int x = 0; x < sizeX; x++)
			{
				for (int y = 0; y < sizeY; y++)
				{
					...
					{
						if (!TryGetDefinition(cells[x, y, z], registry, predicate, out Erelia.Core.Voxel.VoxelDefinition definition, out Erelia.Core.Voxel.Cell cell))
						{
							continue;
						}

						Erelia.Core.Voxel.Shape shape = definition.Shape;
						if (shape == null)
						{
							continue;
						}

						Erelia.Core.Voxel.Shape.FaceSet faceSet = shape.RenderFaces;
						...
						for (int i = 0; i < 6; i++)
						{
							Erelia.Core.Voxel.AxisPlane nonOrientedPlane = (Erelia.Core.Voxel.AxisPlane)(i);

							Erelia.Core.Voxel.AxisPlane localPlane =
								Erelia.Core.Voxel.Utils.Geometry.MapWorldPlaneToLocal(nonOrientedPlane, cell.Orientation, cell.FlipOrientation);

							if (faceSet.OuterShell != null
								&& faceSet.OuterShell.TryGetValue(localPlane, out Erelia.Core.Voxel.Face localFace)
								&& localFace != null
								&& localFace.HasRenderablePolygons)
							{
								if (!IsFaceOccludedByNeighbor(
										...
										useCollision: false))
								{
									Erelia.Core.Voxel.Face transformed =
						...
						if (anyOuterVisible && faceSet.Inner != null)
						{
							...
							{
								Erelia.Core.Voxel.Face innerFace =
		...
		private static bool IsFaceSetEmpty(Erelia.Core.Voxel.Shape.FaceSet faceSet)
		{
			...
			for (int x = 0; x < sizeX; x++)
			{
				for (int y = 0; y < sizeY; y++)
				{
					for (int z = 0; z < sizeZ; z++)
					{
						...
						if (!TryGetDefinition(cells[x, y, z], registry, predicate, out Erelia.Core.Voxel.VoxelDefinition def, out Erelia.Core.Voxel.Cell cell))
						{
							continue;
						}

						Erelia.Core.Voxel.Shape shape = def.Shape;
						if (shape == null)
						{
							continue;
						}

						Erelia.Core.Voxel.Shape.FaceSet fs = shape.CollisionFaces;
		...
		private static bool IsCubicCollisionVoxel(Erelia.Core.Voxel.Cell cell, Erelia.Core.Voxel.Shape shape)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				if (!fs.OuterShell.TryGetValue(localPlane, out Erelia.Core.Voxel.Face localFace)
					|| localFace == null
					|| !localFace.HasRenderablePolygons)
				{
					return false;
				...
				Erelia.Core.Voxel.Face worldFace = TransformFaceCached(localFace, cell.Orientation, cell.FlipOrientation);
				if (!Erelia.Core.Voxel.Utils.Geometry.IsFullFace(worldFace, nonOrientedPlane))
				{
					return false;
				}
			}
			...
			for (int x = 0; x < sizeX; x++)
			{
				for (int y = 0; y < sizeY; y++)
				{
					for (int z = 0; z < sizeZ; z++)
					{
						...
						Erelia.Core.Voxel.Shape.FaceSet faceSet = shape.CollisionFaces;
			...
			List<int> triangles)
		{
			...
			for (int p = 0; p < facePolygons.Count; p++)
			{
				...
				{
					Erelia.Core.Voxel.Face.Vertex v = faceVertices[i];
		...
		private static Erelia.Core.Voxel.Face TransformFaceCached(
			Erelia.Core.Voxel.Face face,
			...
			Erelia.Core.Voxel.FlipOrientation flipOrientation)
		{
			...
			if (Erelia.Core.Voxel.MesherUtils.FaceByOrientationCache.TryGetValue(face, orientation, flipOrientation, out Erelia.Core.Voxel.Face output))
			{
				return output;
			}
		...
		private static bool IsFaceOccludedByNeighbor(
			...
			int z,
			Erelia.Core.Voxel.Cell cell,
			Erelia.Core.Voxel.Face localFace,
			Erelia.Core.Voxel.AxisPlane nonOrientedPlane,
			Erelia.Core.Voxel.VoxelRegistry registry,
			Func<Erelia.Core.Voxel.VoxelDefinition, bool> predicate,
			bool useCollision)
		{
			Vector3Int offset = Erelia.Core.Voxel.Utils.Geometry.PlaneToOffset(nonOrientedPlane);
			int nx = x + offset.x;
			int ny = y + offset.y;
			int nz = z + offset.z;
			...
			if (!TryGetDefinition(cells[nx, ny, nz], registry, predicate, out Erelia.Core.Voxel.VoxelDefinition neighborDefinition, out Erelia.Core.Voxel.Cell neighborCell))
			{
				return false;
			}

			Erelia.Core.Voxel.Shape neighborShape = neighborDefinition.Shape;
			if (neighborShape == null)
			{
				return false;
			}

			Erelia.Core.Voxel.Shape.FaceSet neighborFaceSet = useCollision ? neighborShape.CollisionFaces : neighborShape.RenderFaces;

			Erelia.Core.Voxel.AxisPlane oppositePlane = Erelia.Core.Voxel.Utils.Geometry.GetOppositePlane(nonOrientedPlane);

			Erelia.Core.Voxel.AxisPlane neighborLocalPlane =
				Erelia.Core.Voxel.Utils.Geometry.MapWorldPlaneToLocal(oppositePlane, neighborCell.Orientation, neighborCell.FlipOrientation);

			if (neighborFaceSet.OuterShell == null
				|| !neighborFaceSet.OuterShell.TryGetValue(neighborLocalPlane, out Erelia.Core.Voxel.Face neighborLocalFace)
			...
			Erelia.Core.Voxel.Face faceWorld = TransformFaceCached(localFace, cell.Orientation, cell.FlipOrientation);
			Erelia.Core.Voxel.Face neighborWorld = TransformFaceCached(neighborLocalFace, neighborCell.Orientation, neighborCell.FlipOrientation);
			...
			if (Erelia.Core.Voxel.MesherUtils.FaceVsFaceOcclusionCache.TryGetValue(faceWorld, neighborWorld, out bool occluded))
			{
				return occluded;
			}
			...
			bool useCollision)
		{
			...
			Erelia.Core.Voxel.Face neighborWorld =
			...
			Erelia.Core.Voxel.Face fullFace = Erelia.Core.Voxel.Utils.Geometry.FullOuterFaces[(int)oppositePlane];
			if (fullFace == null)
			{
				return false;
			}

			if (Erelia.Core.Voxel.MesherUtils.FaceVsFaceOcclusionCache.TryGetValue(fullFace, neighborWorld, out bool occluded))
			{
				return occluded;
			}

			return Erelia.Core.Voxel.Utils.Geometry.IsFullFace(neighborWorld, oppositePlane);
		}
	}
}
